...
use utils::wyndex::{ExecuteMsg::ExecuteSwapOperations, SwapOperation};
...
pub fn instantiate(
    ...
    let ctoken_msg = isotonic_token::msg::InstantiateMsg {
...
mod execute {
    use cosmwasm_std::{from_binary, CosmosMsg, SubMsg, coin};
    use cw20::Cw20ReceiveMsg;
    use utils::{
        amount::{base_to_token, token_to_base},
        coin::{Coin, coin_native}, wyndex::SimulateSwapOperationsResponse,
    };
    use wyndex::pair::SpotPricePredictionResponse;
    ...
    pub fn charge_interest<T>(deps: DepsMut, env: Env) -> Result<Ratios<T>, ContractError> {
        ...
        }) = calculate_interest(deps.as_ref(), epochs_passed)?
        {
            ...
            if reserve > Uint128::zero() {
                let mint_msg = to_binary(&isotonic_token::msg::ExecuteMsg::MintBase {
    ...
    pub fn withdraw(
        ...
        // Burn the C tokens
        let burn_msg = to_binary(&isotonic_token::msg::ExecuteMsg::BurnBaseFrom {
    ...
    pub fn transfer_from(
        ...
        let msg = to_binary(&isotonic_token::msg::ExecuteMsg::TransferBaseFrom {
    ...
    pub fn swap_withdraw_from(
        ...
        let send_msg = dbg!(buy.denom.send_msg(sender, buy.amount)?);
        ...
        let estimate = swap_response.amount;

        if estimate > sell_limit {
            return Err(ContractError::EstimateHigherThanLimit {estimate, sell_limit})
        }
        ...
        let swap_msg = dbg!(cfg.market_token.swap_msg(oracle_config.multi_hop, operations, Some(buy.amount), sell_limit)?);
...
mod query {
    ...
    use wyndex::pair::SpotPriceResponse;
    use wyndex_oracle::msg::QueryMsg as OracleQueryMsg;
