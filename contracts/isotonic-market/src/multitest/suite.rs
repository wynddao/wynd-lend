...
use utils::{amount::token_to_base, coin, price::PriceRate};
use wyndex::{
    asset::{Asset, AssetInfo},
    factory::PairType,
    oracle::{SamplePeriod, HALF_HOUR},
    pair::{self, LsdInfo, PairInfo, QueryMsg as PairQueryMsg, StablePoolParams},
};
use wyndex_tests::builder::{WyndexSuite, WyndexSuiteBuilder};

use cosmwasm_std::{
    coin, to_binary, Addr, Binary, Coin, CustomMsg, CustomQuery, Decimal, Empty, StdResult, Uint128,
};
...
use crate::{
    msg::{
        ...
        ReceiveMsg, ReserveResponse, TokensBalanceResponse, TotalDebtResponse,
...
impl SuiteBuilder {
    ...
    /// Sets initial pools. Only pools between `Token::Native` can be created this way.
    ...
    pub fn build(mut self) -> Suite {
        ...
        // Store isotonic-token contract.
        let token_id = app.store_code(contract_token());

        // Instantiate isotonic market contract.
        ...
        /*         for (_, (coin1, coin2)) in self.lsd_pools.clone() {
        ...
        // Distribute initial native tokens to users, isotonic market contract and credit agency.
...
impl Suite {
    ...
    pub fn create_pool_and_provide_liquidity(
        ...
        let resp: PairInfo = self
            ...
            .unwrap();
        assert_eq!(resp.pair_type, pair_type.clone());
    ...
    pub fn mint_tokens_and_allowance(
        ...
        match asset_2.info.clone() {
            ...
            AssetInfo::Native(denom) => {
                ...
                native_tokens.push(coin(asset_2.amount.u128(), denom.clone()));
            }
        };
    ...
    pub fn create_pool_with_liquidity_and_twap_price(
        ...
        self.provide_liquidity(&pair, (coin1.clone(), coin2.clone()))
            .unwrap();
    ...
    /// Shortcut for querying base asset balance in the market contract
    pub fn query_contract_native_balance(&self, denom: String) -> StdResult<u128> {
        self.query_asset_balance(self.contract.as_str(), denom)
    }
    ...
    pub fn query_ctoken_info(&self) -> AnyResult<isotonic_token::msg::TokenInfoResponse> {
        let ctoken = self.ctoken_contract.clone();
        self.app
            .wrap()
            .query_wasm_smart(ctoken, &isotonic_token::msg::QueryMsg::TokenInfo {})
            .map_err(|err| anyhow!(err))
    }
