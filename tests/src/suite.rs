...
use swap_bindings::{Step, Swap, SwapMsg, SwapQuery};
use swap_bindings_test::{Pool, SwapApp};
use utils::{credit_line::CreditLineResponse, token::Token};
use wyndex_oracle::msg::{ExecuteMsg as OracleExecuteMsg, InstantiateMsg as OracleInstantiateMsg};

use isotonic_credit_agency::msg::{
    ExecuteMsg as CAExecuteMsg, InstantiateMsg, IsOnMarketResponse, ListEnteredMarketsResponse,
    ListMarketsResponse, MarketResponse, QueryMsg,
};
use isotonic_credit_agency::state::Config;

use crate::MarketBuilder;

fn contract_osmosis_oracle() -> Box<dyn Contract<SwapMsg, SwapQuery>> {
...
fn contract_credit_agency() -> Box<dyn Contract<SwapMsg, SwapQuery>> {
    let contract = ContractWrapper::new(
...
pub fn contract_market() -> Box<dyn Contract<SwapMsg, SwapQuery>> {
    let contract = ContractWrapper::new(
...
fn contract_token() -> Box<dyn Contract<SwapMsg, SwapQuery>> {
        ...
        for (pool_id, (coin1, coin2)) in self.pools.clone() {
            app.execute_contract(
                ...
                &OracleExecuteMsg::RegisterPool {
                    pool_id,
                    denom1: Token::Native(coin1.denom), // TODO: allow cw20
                    denom2: Token::Native(coin2.denom),
                },
                &[],
            )
            .unwrap();
...
impl Suite {
    pub fn app(&mut self) -> &mut SwapApp {
        &mut self.app
    }
    ...
    pub fn swap_exact_in(&mut self, sender: &str, sell: Coin, buy: &str) -> AnyResult<AppResponse> {
        ...
        self.app.execute(
            ...
            SwapMsg::Swap {
                first: Swap {
                    pool_id: pool1,
                    denom_in: Token::Native(sell.denom),
                    denom_out: Token::Native(common_token),
                },
                route: vec![Step {
                    pool_id: pool2,
                    denom_out: Token::Native(buy.to_string()),
                }],
                amount: swap_bindings::SwapAmountWithLimit::ExactIn {
                    input: sell.amount,
    ...
    pub fn deposit(
        &mut self,
        account: &str,
        tokens: impl Into<utils::coin::Coin>,
        ...
        self.app.execute_contract(
            ...
            &[coin(
                // TODO: assuming native
                tokens.amount.u128(),
                tokens.denom.as_native().unwrap(),
            )],
        )
    }

    pub fn withdraw(&mut self, account: &str, tokens: utils::coin::Coin) -> AnyResult<AppResponse> {
        let market = self.query_market(&tokens.denom)?; // TODO: assuming native
    ...
    pub fn attempt_withdraw_max(&mut self, sender: &str, token: &str) -> AnyResult<()> {
        ...
        assert!(self
            .withdraw(sender, utils::coin::Coin::new_native(1, token))
            .is_err());
    ...
    pub fn attempt_borrow_max(&mut self, sender: &str, token: &str) -> AnyResult<()> {
        ...
        assert!(self.borrow(sender, coin(1, token)).is_err()); // TODO: assuming native
    ...
    pub fn query_withdrawable(
        ...
        denom: &str,
    ) -> AnyResult<utils::coin::Coin> {
        let market = self.query_market(&Token::Native(denom.to_string()))?;
    ...
    /// Queries ctokens balance and debt on market pointed by denom for given account
    pub fn query_tokens_balance(
        &self,
        market_denom: &str,
        account: &str,
    ) -> AnyResult<TokensBalanceResponse> {
        let market = self.query_market(&Token::Native(market_denom.to_string()))?;
    ...
    pub fn query_contract_code_id(&mut self, contract_denom: &str) -> AnyResult<u64> {
        ...
        let market = self.query_market(&Token::Native(contract_denom.to_string()))?;
    ...
    pub fn sudo_adjust_market_id(&mut self, new_market_id: u64) -> AnyResult<AppResponse> {
        ...
        self.app
            .wasm_sudo(contract, &CAExecuteMsg::AdjustMarketId { new_market_id })
    }

    pub fn sudo_adjust_token_id(&mut self, new_token_id: u64) -> AnyResult<AppResponse> {
        let contract = self.credit_agency.clone();
        self.app
            .wasm_sudo(contract, &CAExecuteMsg::AdjustTokenId { new_token_id })
    }
    ...
    pub fn sudo_adjust_common_token(&mut self, new_common_token: &str) -> AnyResult<AppResponse> {
        ...
        self.app.wasm_sudo(
            contract,
            &CAExecuteMsg::AdjustCommonToken {
